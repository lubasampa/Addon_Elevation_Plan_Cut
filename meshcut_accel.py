...
def visibility_mask(scene, accel: VisibilityAcceleration | None, origins: list[Vector], directions: list[Vector], distances: list[float], epsilon: float = 1e-4, require_cython: bool = False):
    ...
    if require_cython and _cy_parallel is None:
        raise RuntimeError(
            "Cython backend is required but not available. Build meshcut_parallel and copy it beside __init__.py"
        )
